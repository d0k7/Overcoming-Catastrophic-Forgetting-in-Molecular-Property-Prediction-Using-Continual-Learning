...
try:
    ...
    model.save_pretrained("/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_Finedtune_Model_4_XAI/CAT_fine_tuned_bert_model")

    # Load fine-tuned model
    model = BertForSequenceClassification.from_pretrained("/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_Finedtune_Model_4_XAI/CAT_fine_tuned_bert_model")

    # Compute Contextual Attention (CAT) Maps
    def compute_cat_maps(model, inputs, attention_mask):
        model.eval()
        inputs_dict = {
            "input_ids": inputs,
        ...
        # Here, CAT will be based on some function of attention scores and logits
        # This is a placeholder, the actual CAT method may involve more complex operations
        cat_maps = [attention.mean(dim=1) for attention in attentions]  # Simplified example
        
        return cat_maps

    # Save Contextual Attention Maps to CSV with tokens
    def save_cat_maps_to_csv(cat_maps, inputs, path):
        decoded_inputs = [tokenizer.convert_ids_to_tokens(input_ids) for input_ids in inputs]
        data = []
        for tokens, cat_map_layer in zip(decoded_inputs, cat_maps[-1]):  # Using the last layer's CAT maps
            for token, cat_score in zip(tokens, cat_map_layer.squeeze().tolist()):  # Assuming cat_map_layer is [batch_size, seq_len]
                data.append([token, cat_score])

        df = pd.DataFrame(data, columns=['Token', 'CAT Score'])
        try:
            os.makedirs(os.path.dirname(path), exist_ok=True)
            df.to_csv(path, index=False)
            print(f"Contextual Attention maps saved to {path} successfully.")
        except Exception as e:
            print(f"Error saving Contextual Attention maps: {e}")

    # Loop over models and datasets for Contextual Attention Maps computation
    results = {}
    ...
    for key in ["C2C"]:
        ...
        cat_maps = compute_cat_maps(model, inputs, attention_mask)

        csv_path = f"/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_XAI_Computed/{key}_CAT_Maps.csv"
        save_cat_maps_to_csv(cat_maps, inputs, csv_path)

        results[key] = cat_maps
    ...
    print(f"An error occurred during training or Contextual Attention map computation: {e}")

if computation_successful:
    print("Computation and saving of Contextual Attention maps completed successfully.")
else:
    print("Computation or saving of Contextual Attention maps failed.")
