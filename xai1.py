...
try:
    ...
    print(e)
    raise ValueError(f"Failed to load the tokenizer: {e}")
...
try:
    ...
    model.save_pretrained("/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_Finedtune_Model_4_XAI/Integrated_Gradient_fine_tuned_bert_model")

    # Load fine-tuned model
    model = BertForSequenceClassification.from_pretrained("/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_Finedtune_Model_4_XAI/Integrated_Gradient_fine_tuned_bert_model")

    # Compute Integrated Gradients
    def compute_integrated_gradients(model, inputs, attention_mask, labels):
        model.eval()
        inputs_dict = {
            "input_ids": inputs,
        ...
        attributions, delta = ig.attribute(inputs_dict, target=labels, return_convergence_delta=True)
        return attributions

    # Save Integrated Gradients to CSV with tokens
    def save_integrated_gradients_to_csv(attributions, inputs, path):
        decoded_inputs = [tokenizer.convert_ids_to_tokens(input_ids) for input_ids in inputs]
        data = []
        for tokens, attribution in zip(decoded_inputs, attributions):
            for token, attr in zip(tokens, attribution.mean(dim=1).squeeze().tolist()):  # Averaging attributions across heads
                data.append([token, attr])

        df = pd.DataFrame(data, columns=['Token', 'Attribution Score'])
        try:
            os.makedirs(os.path.dirname(path), exist_ok=True)
            df.to_csv(path, index=False)
            print(f"Integrated gradients saved to {path} successfully.")
        except Exception as e:
            print(f"Error saving integrated gradients: {e}")

    # Loop over models and datasets for Integrated Gradients computation
    results = {}
    ...
    for key in ["C2C"]:
        ...
        attributions = compute_integrated_gradients(model, inputs, attention_mask, labels)

        csv_path = f"/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/{key}_Integrated_Gradients.csv"
        save_integrated_gradients_to_csv(attributions, inputs, csv_path)

        results[key] = attributions

except Exception as e:
    computation_successful = False
    print(f"An error occurred during training or integrated gradients computation: {e}")

if computation_successful:
    print("Computation and saving of integrated gradients completed successfully.")
else:
    print("Computation or saving of integrated gradients failed.")
