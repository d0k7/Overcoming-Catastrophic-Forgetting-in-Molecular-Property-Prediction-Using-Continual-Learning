...
try:
    ...
    model.save_pretrained("/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_Finedtune_Model_4_XAI/AttCAT_fine_tuned_bert_model")

    # Load fine-tuned model
    model = BertForSequenceClassification.from_pretrained("/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_Finedtune_Model_4_XAI/AttCAT_fine_tuned_bert_model")

    # Compute Attention-weighted Contextual Attention (AttCAT) Maps
    def compute_attcat_maps(model, inputs, attention_mask):
        model.eval()
        inputs_dict = {
            "input_ids": inputs,
        ...
        print("Attention map dimensions:", [attention.shape for attention in attentions])
        print("Logits dimensions before reshape:", logits.shape)
        
        # Reshape logits to match the attention map dimensions
        logits_expanded = logits.unsqueeze(1).unsqueeze(-1).expand(-1, attentions[-1].size(1), -1, attentions[-1].size(-1))
        
        # Print dimensions after expansion
        print("Logits dimensions after expansion:", logits_expanded.shape)
        
        # Compute AttCAT maps
        attcat_maps = [attention * logits_expanded[:, :, 1, :].unsqueeze(2) for attention in attentions]
        
        return attcat_maps

    # Save Attention-weighted Contextual Attention Maps to CSV with tokens
    def save_attcat_maps_to_csv(attcat_maps, inputs, path):
        decoded_inputs = [tokenizer.convert_ids_to_tokens(input_ids) for input_ids in inputs]
        data = []
        for tokens, attcat_layer in zip(decoded_inputs, attcat_maps[-1]):  # Using the last layer's AttCAT maps
            for token, attcat_score in zip(tokens, attcat_layer.mean(dim=1).squeeze().tolist()):  # Averaging attention heads
                data.append([token, attcat_score])

        df = pd.DataFrame(data, columns=['Token', 'AttCAT Score'])
        try:
            os.makedirs(os.path.dirname(path), exist_ok=True)
            df.to_csv(path, index=False)
            print(f"Attention-weighted Contextual Attention maps saved to {path} successfully.")
        except Exception as e:
            print(f"Error saving Attention-weighted Contextual Attention maps: {e}")

    # Loop over models and datasets for Attention-weighted Contextual Attention Maps computation
    results = {}
    ...
    for key in ["C2C"]:
        ...
        attcat_maps = compute_attcat_maps(model, inputs, attention_mask)

        csv_path = f"/scratch/sakshi.rs.cse21.itbhu/dheeraj/XAI/XAI1/All_XAI_Computed/{key}_AttCAT_Maps.csv"
        save_attcat_maps_to_csv(attcat_maps, inputs, csv_path)

        results[key] = attcat_maps
...
# This part should follow the previous code without interruption.

except Exception as e:
    computation_successful = False
    print(f"An error occurred during training or Attention-weighted Contextual Attention map computation: {e}")
    raise

if computation_successful:
    print("Computation and saving of Attention-weighted Contextual Attention maps completed successfully.")
else:
    print("Computation or saving of Attention-weighted Contextual Attention maps failed.")
